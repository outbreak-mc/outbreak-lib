...
import space.outbreak.lib.utils.locale.db.DEFAULT_TABLE_NAMES
...
class DbModuleTests {
    companion object {
        ...
        @JvmStatic
        fun prepareTestDB() {
            LocaleDataManagerBase.initDatabaseTables(database)
            transaction(database) {
                DEFAULT_TABLE_NAMES.tables.locale.let { t ->
                    fun insert(key: String, value: String, lang: Locale = "ru_RU") {
                        t.insert {
    ...
    fun tablesInitialization() {
        ...
        val firstTime = measureTime { LocaleDataManagerBase.initDatabaseTables(database) }.inWholeMilliseconds
        val secondTime = measureTime { LocaleDataManagerBase.initDatabaseTables(database) }.inWholeMilliseconds
        val thirdTime = measureTime { LocaleDataManagerBase.initDatabaseTables(database) }.inWholeMilliseconds
    ...
    fun testenumlocale() {
        prepareTestDB()
        LocaleDataManagerBase.loadAllFromDB(database, listOf(NAMESPACE))
